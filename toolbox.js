// Had to make another file for this, as the default one allows multiple selections, we don't
// so we overwrite the old suggestion with new choices
function elk_ToolBox(oOptions)
{
	this.opt = oOptions;
...
elk_ToolBox.prototype.init = function()
{
	this.oToAutoSuggest = new smc_AutoSuggest({
		...
		sItemTemplate: '<input type="hidden" name="%post_name%" value="%item_id%" /><a href="%item_href%" class="extern" onclick="window.open(this.href, \'_blank\'); return false;">%item_name%</a>&nbsp;<img src="%images_url%/pm_recipient_delete.png" alt="%delete_text%" title="%delete_text%" onclick="return %self%.deleteAddedItem(%item_id%);" /> ' + this.opt.sTextViewItem,
...
elk_ToolBox.prototype.callbackAddItem = function(oAutoSuggestInstance, sSuggestId)
{
